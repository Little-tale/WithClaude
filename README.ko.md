...
이렇게 하면 기본적으로 `@designGemini`와 `@reviewGemini` 모두 같은 Gemini CLI alias를 사용합니다.
...
oh-my-openagent / oh-my-opencode 스타일의 agent override도 함께 사용한다면, 대상 에이전트가 `with-claude/*` 모델을 명시적으로 가리키도록 설정하면 됩니다.
...
이 설정은 `with-claude` provider가 이미 설치되어 있고 OpenCode provider 설정에 등록되어 있을 때만 동작합니다.
